...
struct ChessBoard{
    ...
    void setdifficulty(string diff) {
        if(diff == "e") difficulty_level = 2;
        else if(diff == "m") difficulty_level = 4;
        else difficulty_level = 6;
    }
    ...
    */
    int score(){
        ...
        for(auto & p : white_pieces)
        {
            if(p.second == Piece::king) add = kingEvalWhite[p.first.x][p.first.y];
            else if(p.second == Piece::queen) add = evalQueen[p.first.x][p.first.y];
            else if(p.second == Piece::white_pawn) add = pawnEvalWhite[p.first.x][p.first.y];
            else if(p.second == Piece::rook) add = rookEvalWhite[p.first.x][p.first.y];
            else if(p.second == Piece::bishop) add = bishopEvalWhite[p.first.x][p.first.y];
            else if(p.second == Piece::knight) add = knightEval[p.first.x][p.first.y];
            sumWhite += pieceValues[p.second] + add;
            add = 0;
        }
        int sumBlack = 0;
        for(auto & p : black_pieces)
        {
            if(p.second == Piece::king) add = kingEvalBlack[p.first.x][p.first.y];
            else if(p.second == Piece::queen) add = evalQueen[p.first.x][p.first.y];
            else if(p.second == Piece::black_pawn) add = pawnEvalBlack[p.first.x][p.first.y];
            else if(p.second == Piece::rook) add = rookEvalBlack[p.first.x][p.first.y];
            else if(p.second == Piece::bishop) add = bishopEvalBlack[p.first.x][p.first.y];
            else if(p.second == Piece::knight) add = knightEval[p.first.x][p.first.y];
            sumBlack += pieceValues[p.second] + add;
            add = 0;
        }
        return sumWhite-sumBlack;
    }
        ...
        if(minimize == false) {
            for(auto & from : moverPieces()){
                for(auto & to : possibleMoves(from.first)){
                    ...
                    if(best_move.score >= beta) break;// best_move;
            ...
            for(auto & from : moverPieces()){
                for(auto & to : possibleMoves(from.first)){
                    ...
                    if(best_move.score <= alpha) break;// best_move;
    ...
    void AIMove(){
        ...
        int alpha = -inf;
        int beta = inf;
        Move m = minimax(difficulty_level,minimize,alpha,beta);                               /// Depth = 4. Easy Medium Hard.
        makeMove(m.from,m.to);
        printBoard();
    }
};

map<ChessBoard::Piece,int> ChessBoard::pieceValues {{ChessBoard::Piece::king, 9000},
{ChessBoard::Piece::queen, 900}, {ChessBoard::Piece::black_pawn, 100}, {ChessBoard::Piece::white_pawn, 100},
{ChessBoard::Piece::bishop, 300},{ChessBoard::Piece::knight, 300},{ChessBoard::Piece::rook, 500},};
